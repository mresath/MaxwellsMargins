...
> **Status**: All six phases are implemented and interactively verified - app shell, electrostatics, magnetism (uniform field, current wires, charged particles), induction (moving/rotating loops, Faraday/Lenz, the generator demo), circuits (resistor/capacitor/inductor/battery/switch/probe components solved via modified nodal analysis, with RC/RL/LC transients), and cross-cutting polish (per-quantity logging/graphing, a unit-display/overlay-toggle audit, the app icon). See [PLAN.md](PLAN.md) for the phased build order.
...
#### `DipoleMagnet.hpp`
- A stationary permanent magnet whose dipole axis points through the page (matching `CurrentLoop`'s model, not a bar magnet lying flat in the page - see PLAN.md's open decisions log for why): center, radius, signed `surfaceField` (Tesla, + = North pole toward the viewer), stable `id`. Trivial state holder; field math (`FieldMath::dipoleMagnetField`) and rendering (a solid disc tinted by pole) live elsewhere.
...
- **Current status**: builds and links a complete app - app shell, electrostatics, magnetism, induction, circuits, and cross-cutting polish (Phases 1-6, see PLAN.md)
